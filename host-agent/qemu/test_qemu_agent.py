...
class SharedWiringTest(unittest.TestCase):
    """The behaviour lives in host-agent/shared/test_agent_common.py; this pins
    that qemu-agent.py actually re-exports it (issue #126 is one of the bugs
    that only existed because the two agents carried separate copies)."""

    def test_helpers_are_the_shared_ones(self):
        for name in (
            "host_authority", "resolve_public_host", "now_iso", "sanitize_name",
            "pid_alive", "run", "sudo", "encode_frame", "parse_attach_spec",
            "drain_buffered", "set_winsize", "FrameDecoder", "HTTPError",
        ):
            self.assertIs(
                getattr(qemu_agent, name), getattr(qemu_agent.common, name), name
            )

    def test_url_composition_brackets_ipv6(self):
        self.assertEqual(
    ...
    def test_ssh_client_uses_agent_paths(self):
        self.assertIn(str(qemu_agent.SSH_KEY), qemu_agent.SSH.base)
        self.assertEqual(
            qemu_agent._ssh_control_path("10.201.1.2"),
            str(qemu_agent.SSH_CONTROL_DIR / "10.201.1.2.sock"),
        )
        self.assertIs(qemu_agent.ssh_exec.__self__, qemu_agent.SSH)

    def test_unknown_vm_attach_is_a_404(self):
        with mock.patch.object(qemu_agent, "load_meta", return_value=None):
            with self.assertRaises(qemu_agent.HTTPError) as cm:
                qemu_agent.do_attach(object(), "vm-missing", {"tty": True, "cmd": []})
