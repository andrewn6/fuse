...
| `--check-secrets` | Also fail when a secret the Fusefile requires is missing from the resolved set. An empty value counts as missing, matching [`fuse up`](/reference/cli/up#secrets). |
...
[`fuse up`](/reference/cli/up#path-resolution): `-f/--file` if set, then a
positional argument, then the first Fusefile discovered in the working
directory. ([`fuse init`](/reference/cli/init) differs: it picks a write
