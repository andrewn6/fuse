...
| `--arch` | Host CPU architecture, `amd64` or `arm64`, override. Empty (default) probes the host agent. A declared value that contradicts the probe is rejected. |
...
| `--label` | Placement label as `key=value`, for example `disk=nvme`. Repeatable. Declared, never probed. |
...
See [Scheduling and placement](/docs/concepts/scheduling).
