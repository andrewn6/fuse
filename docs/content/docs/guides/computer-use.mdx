...
guest token from the orchestrator to the agent). What it does not give you
yet: per-environment egress policy — the browser can reach whatever the host
can. Treat the environment as a blast-radius boundary, not a network policy,
and keep secrets out of desktop environments a model is driving.
